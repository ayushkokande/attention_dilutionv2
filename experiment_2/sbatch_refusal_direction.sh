...
#
# Layer-selection judge config (see experiment_2/refusal_direction.py
# docstring): 24-token greedy + 26-phrase Suraj substring matcher inside the
# causal sweep only. Stricter than the project-default 128-token / 17-phrase
# behavioral judge used downstream — long-window judge over-counts tail-refusal
# at causally-effective layers and biases canonical pick toward early layers.
# Expected canonical for Qwen3-14B under this judge: L36 (closest causally-
# effective layer to peak-norm L39); previous runs at 128-token judge produced
# L18 (uniquely zero post-ablate) for the same model+pool.
#
# Outputs (read by experiment_3 / experiment_8 / experiment_9):
...
#
# Run:
#   bash experiment_2/sbatch_refusal_direction.sh
...
LOG_OUT="${REPO}/logs/qwen3_14b_refdir_${JOB_ID}.out"
LOG_ERR="${REPO}/logs/qwen3_14b_refdir_${JOB_ID}.err"
...
if [ "$missing" = "1" ]; then
  echo >&2
  echo "Sync the repo with git or rsync if scripts are missing." >&2
  exit 1
fi
...
echo "=== qwen3_14b_refusal_direction | job ${JOB_ID} ==="
echo "Repo : $(pwd)"
